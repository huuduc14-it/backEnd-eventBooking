    ...
    try {
      ...
      for (const item of items) {
        ...
        if (ticketType.remaining < item.quantity) {
          throw new Error(
            `Loại vé ${item.ticket_type_id} chỉ còn ${ticketType.remaining} vé`
          );
        }
  ...
  cancelBooking: async (bookingId, userId) => {
    ...
    try {
      ...
      return {
        success: true,
        message:
          "Hủy vé thành công. Số tiền sẽ được hoàn trả trong 3-5 ngày làm việc.",
      };
    } catch (error) {
      await connection.rollback();
      throw error;
  ...
  updateTicketQuantity: async (userId, ticketTypeId, newQuantity) => {
    ...
    try {
      ...
      if (newQuantity < soldQuantity) {
        throw new Error(
          `Không thể giảm xuống dưới ${soldQuantity} vé (số vé đã bán)`
        );
      }
