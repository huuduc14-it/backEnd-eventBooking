const TicketTypeModel = require("../models/ticket_type_Model.js");
...
async function checkOwner(userId, ticketTypeId) {
  const sql = `
    ...
    `;
  const [rows] = await db.execute(sql, [ticketTypeId]);
...
//     const isOwner = await checkOwner(req.user.user_id, ticketTypeId);

//     if (!isOwner) {
//       res.status(403).json({ success: false, msg: "Not Authorized " });
//     }
//     await TicketTypeModel.update(ticketTypeId, {
//       name,
//       price,
//       total_quantity,
//     });
...
exports.updateTicket = async (req, res) => {
  try {
    ...
    if (Object.keys(updateData).length === 0) {
      return res.status(400).json({ success: false, msg: "No data to update" });
    }

    await TicketTypeModel.update(ticketTypeId, updateData);
...
exports.addTicket = async (req, res) => {
  try {
    ...
    if (events.length === 0 || events[0].user_id !== req.user.user_id) {
      return res
        .status(403)
        .json({ success: false, message: "Not Authorized" });
    }

    const newId = await TicketTypeModel.add(event_id, {
      name,
      price,
      total_quantity,
      description,
